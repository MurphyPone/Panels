...
var client_id = "8df0d68fcc1920c92fc389b89e7ce20f"; //For SoundCloud API
...
function frameLooper() {
  resize_canvas(); //resize the canvas every update or else the framerate decreases over time

  var grd = ctx.createLinearGradient(0, 0, 0, canvas.height);
  grd.addColorStop(0, "rgba(100, 100, 100, 1)");
  grd.addColorStop(1, "rgba(51, 51, 51, 1)");
  ...
  center_x = 2 * canvas.width / 3 - (react_x * 0.007);
  ...
  if (deltarad > 15) {
    shockwave = 0;
    ctx.fillStyle = "rgba(255, 255, 255, 0.7)"; //flash low opacity white sceen
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    rot = rot + 0.4;
  }
